...
export default function BookingForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // 1. Validácia pred guardom — aby ref neostal zablokovaný pri chybe
    const err = validateContact(customerContact);
    setContactError(err);
    if (err) return;

    // 2. Guard proti double-submitu (synchrónny ref, nie Apollo loading)
    ...
    try {
      ...
      if (errors.length > 0) {
        setServerErrors(errors.map((e) => e.message));
        return;
      }
